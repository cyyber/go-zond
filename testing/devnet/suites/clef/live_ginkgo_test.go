...
var _ = ginkgo.Describe(
	...
	func() {
		...
		ginkgo.BeforeAll(func(ctx ginkgo.SpecContext) {
			...
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			gomega.Expect(chainID.Cmp(big.NewInt(defaultChainID))).To(gomega.Equal(0))
		...
		ginkgo.It("signs and verifies QRL typed data", func(ctx ginkgo.SpecContext) {
			gomega.Expect(
				verifyTypedDataSigning(ctx, session.client, session.account, session.expectedWallet),
			).To(gomega.Succeed())
		}, ginkgo.SpecTimeout(liveSpecTimeout))

		ginkgo.It("signs, submits, and confirms a VM64 transaction", func(ctx ginkgo.SpecContext) {
			...
			request := transactionArgs(session.account, chainID, nonce, tip, feeCap)
