...
func TestGolangBindings(t *testing.T) {
	...
	// Forward the go-qrllib replace from the root go.mod so the bindtest
	// temp module resolves the 48-byte-address fork rather than the upstream
	// go-qrllib with its legacy 20-byte address type.
	rootGoMod, err := os.ReadFile(filepath.Join(pwd, "..", "..", "..", "go.mod"))
	if err != nil {
