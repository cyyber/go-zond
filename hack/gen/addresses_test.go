...
func TestAddressesDeterministic(t *testing.T) {
	...
	// Every address must be a well-formed Q-prefixed hex string the length
	// of the current common.AddressLength.
	wantLen := 1 + 2*common.AddressLength
	for _, acc := range accounts {
		if len(acc.Address) != wantLen {
			t.Errorf("%s: address wrong length %d, want %d", acc.Label, len(acc.Address), wantLen)
		}
		if acc.Address[0] != 'Q' {
