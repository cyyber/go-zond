...
[![Go Report Card](https://goreportcard.com/badge/github.com/theQRL/go-qrl)](https://goreportcard.com/report/github.com/theQRL/go-qrl)
...
The go-qrl project comes with several wrappers/executables found in the `cmd`
directory.
...
|  `abigen`   | Source code generator to convert QRL contract definitions into easy-to-use, compile-time type-safe Go packages. It operates on plain QRL contract ABIs with expanded functionality if the contract bytecode is also available. However, it also accepts Hyperion source files, making development much more streamlined. Please see `abigen --help` for details.                                            |
...
As an alternative to passing the numerous flags to the `gqrl` binary, you can also pass a
configuration file via:

```shell
$ gqrl --config /path/to/your_config.toml
```
...
```shell
$ gqrl --your-favourite-flags dumpconfig
```
...
and/or hosts. By default, `gqrl` binds to the local interface and RPC endpoints are not
accessible from the outside.

### Programmatically interfacing `gqrl` nodes
...
this, `gqrl` has built-in JSON-RPC APIs for QRL. Some method names follow
...
The IPC interface is enabled by default and exposes all the APIs supported by `gqrl`,
...
connect via HTTP, WS or IPC to a `gqrl` node configured with the above flags and you'll
...
If you'd like to contribute to go-qrl, please fork, fix, commit and send a pull request
...
The go-qrl library (i.e. all code outside of the root `cmd` directory and
`testing/devnet/cmd`) is licensed under the
[GNU Lesser General Public License v3.0](https://www.gnu.org/licenses/lgpl-3.0.en.html),
also included in our repository in the `COPYING.LESSER` file.

The go-qrl binaries (i.e. all code inside of the root `cmd` directory and
`testing/devnet/cmd`) are licensed under the
