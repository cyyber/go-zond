...
type downloadTester struct {
	...
	stateDb zonddb.Database // Database used by the tester for syncing from peers
	peerDb  zonddb.Database // Database of the peers containing all data
...
func testSyncProgress(t *testing.T, protocol uint, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", zond.SyncProgress{
	...
	progress <- struct{}{}
	pending.Wait()
	checkProgress(t, tester.downloader, "final", zond.SyncProgress{
		StartingBlock: uint64(chain.len()/2 - 1),
...
func testForkedSyncProgress(t *testing.T, protocol uint, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "forking", zond.SyncProgress{
