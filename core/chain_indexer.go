...
type ChainIndexer struct {
	chainDb  zonddb.Database     // Chain database to index the data from
	indexDb  zonddb.Database     // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb zonddb.Database, indexDb zonddb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
