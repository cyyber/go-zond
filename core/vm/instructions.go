...
func opAdd(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	x, y := scope.Stack.pop(), scope.Stack.peek()
	y.Add(&x, y)
	return nil, nil
}

func opSub(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	x, y := scope.Stack.pop(), scope.Stack.peek()
	y.Sub(&x, y)
	return nil, nil
}

func opMul(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	x, y := scope.Stack.pop(), scope.Stack.peek()
	y.Mul(&x, y)
	return nil, nil
}

func opDiv(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	x, y := scope.Stack.pop(), scope.Stack.peek()
	y.Div(&x, y)
	return nil, nil
}

func opSdiv(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	x, y := scope.Stack.pop(), scope.Stack.peek()
	y.SDiv(&x, y)
	return nil, nil
}

func opMod(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	x, y := scope.Stack.pop(), scope.Stack.peek()
	y.Mod(&x, y)
	return nil, nil
}

func opSmod(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	x, y := scope.Stack.pop(), scope.Stack.peek()
	y.SMod(&x, y)
	return nil, nil
}

func opExp(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	base, exponent := scope.Stack.pop(), scope.Stack.peek()
	exponent.Exp(&base, exponent)
	return nil, nil
}

func opSignExtend(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	back, num := scope.Stack.pop(), scope.Stack.peek()
	num.ExtendSign(num, &back)
	return nil, nil
}

func opNot(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	x := scope.Stack.peek()
	x.Not(x)
	return nil, nil
}

func opLt(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	x, y := scope.Stack.pop(), scope.Stack.peek()
	if x.Lt(y) {
		y.SetOne()
	} else {
...
func opGt(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	x, y := scope.Stack.pop(), scope.Stack.peek()
	if x.Gt(y) {
		y.SetOne()
	} else {
...
func opSlt(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	x, y := scope.Stack.pop(), scope.Stack.peek()
	if x.Slt(y) {
		y.SetOne()
	} else {
...
func opSgt(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	x, y := scope.Stack.pop(), scope.Stack.peek()
	if x.Sgt(y) {
		y.SetOne()
	} else {
...
func opEq(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	x, y := scope.Stack.pop(), scope.Stack.peek()
	if x.Eq(y) {
		y.SetOne()
	} else {
...
func opIszero(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	x := scope.Stack.peek()
	if x.IsZero() {
		x.SetOne()
	} else {
...
func opAnd(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	x, y := scope.Stack.pop(), scope.Stack.peek()
	y.And(&x, y)
	return nil, nil
}

func opOr(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	x, y := scope.Stack.pop(), scope.Stack.peek()
	y.Or(&x, y)
	return nil, nil
}

func opXor(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	x, y := scope.Stack.pop(), scope.Stack.peek()
	y.Xor(&x, y)
	return nil, nil
}

func opByte(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	th, val := scope.Stack.pop(), scope.Stack.peek()
	val.Byte(&th)
	return nil, nil
}

func opAddmod(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
...
func opMulmod(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	x, y, z := scope.Stack.pop(), scope.Stack.pop(), scope.Stack.peek()
	z.MulMod(&x, &y, z)
	return nil, nil
}
...
func opKeccak256(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	qrvm := interpreter.qrvm
	if qrvm.Config.EnablePreimageRecording {
		qrvm.StateDB.AddPreimage(interpreter.hasherBuf, data)
	}
...
func opCallDataCopy(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	var (
...
func opReturnDataCopy(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	var (
		memOffset  = scope.Stack.pop()
		dataOffset = scope.Stack.pop()
		length     = scope.Stack.pop()
...
func opCodeCopy(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	var (
		memOffset  = scope.Stack.pop()
...
func opExtCodeCopy(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	var (
		stack      = scope.Stack
		a          = stack.pop()
		memOffset  = stack.pop()
...
// accounts on mainnet have been transferred 1 planck, so the return here should be
...
func opExtCodeHash(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	if interpreter.qrvm.StateDB.Empty(address) {
		...
		slot.SetBytes(interpreter.qrvm.StateDB.GetCodeHash(address).Bytes())
	}
	return nil, nil
}
...
func opBlockhash(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	upper = interpreter.qrvm.Context.BlockNumber.Uint64()
	if upper < 257 {
		lower = 0
	} else {
		lower = upper - 256
	...
	if num64 >= lower && num64 < upper {
		num.SetBytes(interpreter.qrvm.Context.GetHash(num64).Bytes())
	} else {
		num.Clear()
	}
	return nil, nil
...
func opPop(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	scope.Stack.pop()
	return nil, nil
}
...
func opMstore8(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
...
func opSstore(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
...
func opJump(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	if interpreter.qrvm.abort.Load() {
...
func opJumpi(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	if interpreter.qrvm.abort.Load() {
		return nil, errStopToken
	}
...
func opJumpdest(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	return nil, nil
}
...
func opCreate(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	if interpreter.readOnly {
		return nil, ErrWriteProtection
	}
	var (
...
func opCreate2(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	if interpreter.readOnly {
		return nil, ErrWriteProtection
	}
	var (
	...
	res, addr, returnGas, suberr := interpreter.qrvm.Create2(scope.Contract, input, gas,
		bigEndowment, &salt)
...
func opCall(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	stack := scope.Stack
	// Pop gas. The actual gas in interpreter.qrvm.callGasTemp.
	...
	ret, returnGas, err := interpreter.qrvm.Call(scope.Contract, toAddr, args, gas, bigVal)
...
func opDelegateCall(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	stack := scope.Stack
	// Pop gas. The actual gas is in interpreter.qrvm.callGasTemp.
	...
	ret, returnGas, err := interpreter.qrvm.DelegateCall(scope.Contract, toAddr, args, gas)
	if err != nil {
		temp.Clear()
	} else {
		temp.SetOne()
...
func opStaticCall(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	// Pop gas. The actual gas is in interpreter.qrvm.callGasTemp.
	stack := scope.Stack
	// We use it as a temporary value
	temp := stack.pop()
	...
	ret, returnGas, err := interpreter.qrvm.StaticCall(scope.Contract, toAddr, args, gas)
	if err != nil {
		temp.Clear()
	} else {
		temp.SetOne()
...
func opReturn(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	offset, size := scope.Stack.pop(), scope.Stack.pop()
...
func opRevert(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	offset, size := scope.Stack.pop(), scope.Stack.pop()
	ret := scope.Memory.GetPtr(int64(offset.Uint64()), int64(size.Uint64()))
...
func opUndefined(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	return nil, &ErrInvalidOpCode{opcode: OpCode(scope.Contract.Code[*pc])}
}

func opStop(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
	return nil, errStopToken
}
...
func makeLog(size int) executionFunc {
	return func(pc *uint64, interpreter *QRVMInterpreter, scope *ScopeContext) ([]byte, error) {
		...
		interpreter.qrvm.StateDB.AddLog(&types.Log{
			Address: scope.Contract.Address(),
			Topics:  topics,
			Data:    d,
			// This is a non-consensus field, but assigned here because
			// core/state doesn't know the current block number.
			BlockNumber: interpreter.qrvm.Context.BlockNumber.Uint64(),
		})
