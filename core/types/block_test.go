...
// produced pre-migration; with 48-byte addresses and the wider VM word the
...
func TestMaxBlockSize(t *testing.T) {
	...
	// Block layout widened for 48-byte addresses (Coinbase, tx To fields,
	// bloom topic words). Re-run the test and print if that ever changes again.
	const expectedBlockSize = uint64(6951654)
