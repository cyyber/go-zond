...
func (t *prestateTracer) CaptureStart(env *vm.QRVM, from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
	t.env = env
	t.create = create
	t.to = to
...
// CaptureState implements the QRVMLogger interface to trace a single step of VM execution.
...
func (t *prestateTracer) CaptureTxEnd(restGas uint64) {
	...
	for addr, state := range t.pre {
		...
		modified := false
		postAccount := &account{Storage: make(map[common.Hash]common.StorageValue)}
