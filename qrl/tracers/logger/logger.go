...
// Config are the configuration options for structured logger the QRVM
type Config struct {
...
func (l *StructLogger) CaptureStart(env *vm.QRVM, from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
	l.env = env
}
...
func (l *StructLogger) CaptureState(pc uint64, op vm.OpCode, gas, cost uint64, scope *vm.ScopeContext, rData []byte, depth int, err error) {
	...
	// create a new snapshot of the QRVM.
	log := StructLog{pc, op, gas, cost, mem, memory.Len(), stck, rdata, storage, depth, l.env.StateDB.GetRefund(), err}
	l.logs = append(l.logs, log)
}

// CaptureFault implements the QRVMLogger interface to trace an execution fault
...
func (t *mdLogger) CaptureStart(env *vm.QRVM, from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
	t.env = env
	if !create {
...
// ExecutionResult groups all structured logs emitted by the QRVM
...
// StructLogRes stores a structured log emitted by the QRVM while replaying a
...
// formatLogs formats QRVM returned structured logs for json output
